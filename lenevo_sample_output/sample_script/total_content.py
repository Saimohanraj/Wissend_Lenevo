...
for i in range(1, data_sheet.max_row + 1):
    ...
    try:
        ...
        if not os.path.isfile("full_content.csv"):
            df.to_csv("full_content.csv",index=False,mode="a",header=True,encoding="utf_8_sig",)
        else:  # else it exists so append without writing the header
            df.to_csv("full_content.csv",index=False,mode="a",header=False,encoding="utf_8_sig",)
